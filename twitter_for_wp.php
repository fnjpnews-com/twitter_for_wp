    ...
    function on_post_publish($id, $post)
    {
        ...
        if ($post->post_title != null) {
            ...
            if (isset($userInfo->id_str)) {
                ...
                unset($show_text["tweet"]["list"][$num - 5]);
                update_option('showtext_options', $show_text);
            }
        }
    }
        ...
        if ($show_text['oauth_token']) {
            ...
            foreach ($show_text["tweet"]["list"] as $i => $tweet) {
                if ($tweet["id"]) {
                    ...
                    if (floor($tweet_data->retweet_count / 100) * 100 > 0) {
                        if (floor($tweet_data->retweet_count / 100) * 100 > floor($tweet["rt"] / 100) * 100) {
                            $f = floor($tweet_data->retweet_count / 100) * 100;
                            $tweet_str = "【" . (string)$f . "RT】\n" . $tweet_data->text;
                    ...
                    $show_text["tweet"]["list"][$i]["rt"] = $tweet_data->retweet_count;
                }
            }
            update_option('showtext_options', $show_text);
        }
